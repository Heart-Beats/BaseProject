 ...
 */
abstract class BaseMultiLoaderPluginManager(context: Context) : PluginManagerThatSupportMultiLoader(context) {
	...
	/**
	 * 每个插件包的 [pluginKey|uuid] 与其插件加载服务端接口的映射
	 */
	private val pluginLoaderMap = mutableMapOf<String, PluginLoader>()
	 ...
	 */
	abstract fun getPluginProcessServiceName(): String
	...
	private fun loadPlugin(uuid: String, partKey: String) {
		val logIdentity = "pluginKey=$pluginKey|uuid=$uuid"
		Log.d(TAG, "loadPlugin ------> ${logIdentity}, partKey=${partKey}")
	...
	protected open fun startBindPluginProcessService(pluginProcessServiceName: String) {
		if (mPpsController == null) {
			...
			Log.d(TAG, "startBindPluginProcessService: PPS 已绑定 ")
		}
	}

	protected open fun startLoadRunTime(uuid: String) {
		Log.d(TAG, "载入 RunTime ---------------")
		loadRunTime(uuid)    // runtime 已加载之后不会进行重新加载， 即 PluginProcessService 的 mUuid 不会被更改
		Log.d(TAG, "--------------- RunTime  加载结束")
	}

	protected open fun startLoadPluginLoader(uuid: String) {
		Log.d(TAG, "载入 Loader -------------------")
	...
	protected open fun startLoadPlugin(uuid: String, partKey: String) {
		...
		if (!map.containsKey(partKey)) {
			...
			Log.d(TAG, "startLoadPlugin: 开始加载插件包 $logIdentity 中的插件( partKey-${partKey})")
			mPluginLoader.loadPlugin(partKey)
		} else {
			Log.d(TAG, "startLoadPlugin: 插件包 $logIdentity 中的插件( partKey-${partKey}) 已经加载，无需再次加载")
		}

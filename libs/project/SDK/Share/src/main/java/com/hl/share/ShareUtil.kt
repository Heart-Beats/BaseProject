...
import com.hl.share.utils.file2Uri
import com.hl.share.utils.isSingle
import com.hl.share.utils.toArrayList
...
object ShareUtil {
    ...
    fun shareFileWithText(context: Context, text: String, vararg filePaths: String) {
        ...
        val shareIntent = getShareIntent(context, text, filePaths = filePaths)
    ...
    fun shareFile(context: Context, vararg filePaths: String) {
        ...
        val shareIntent = getShareIntent(context = context, filePaths = filePaths)
        context.startActivity(Intent.createChooser(shareIntent, ""))
    }

    private fun getShareIntent(context: Context, text: String? = null, vararg filePaths: String): Intent {
        ...
        val shareIntent: Intent = Intent().apply {
            ...
            this.setShareData(context, shareFiles)
     ...
     * 设置分享的数据
     */
    private fun Intent.setShareData(context: Context, shareFiles: List<File>) {
        val fileUris = shareFiles.map { context.file2Uri(it) }.toArrayList()
    ...
    private fun Intent.setShareContent(context: Context, title: String, imageFile: File) {
        this.putExtra(Intent.EXTRA_TITLE, title)  // 设置描述内容
        this.data = context.file2Uri(imageFile)  // 设置缩略图

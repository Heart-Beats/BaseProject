 ...
 */
class OrientationSensorHelper(lifecycle: Lifecycle, private val angleListener: (x: Float, y: Float, z: Float) -> Unit) : SensorEventListener {
	...
	override fun onSensorChanged(event: SensorEvent) {
		if (System.currentTimeMillis() - lastTime < timeSensor) {
			return
		}
		when (event.sensor.type) {
			Sensor.TYPE_ORIENTATION -> {
				var x = event.values[0]
				x += getScreenRotationOnPhone(BaseUtil.app).toFloat()
				x %= 360.0f
	...
	private fun getScreenRotationOnPhone(context: Context): Int {
		val display = if (BuildVersionUtil.isOver11()) {
			context.display
		} else {

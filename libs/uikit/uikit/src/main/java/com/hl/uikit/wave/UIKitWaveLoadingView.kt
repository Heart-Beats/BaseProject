 ...
 */
class UIKitWaveLoadingView : View {

	companion object Config {
		val TAG = UIKitWaveLoadingView::class.java.simpleName
	...
	private val waveValueAnimUpdateListener = ValueAnimator.AnimatorUpdateListener { animation ->
	 ...
	 * 是否应该加载波浪动画
	 */
	private var isWaveLoadingFlag = true
	...
	private fun init(context: Context, attrs: AttributeSet?) {
		...
		//初始化属性
		context.obtainStyledAttributes(attrs, R.styleable.UIKitWaveLoadingView).also {
			shape = when (it.getInteger(R.styleable.UIKitWaveLoadingView_uikit_wl_shape, DEFAULT_SHAPE)) {
				0 -> Shape.CIRCLE
				1 -> Shape.SQUARE
				2 -> Shape.RECT
				else -> Shape.NONE
			}
			shapeCorner = it.getDimension(R.styleable.UIKitWaveLoadingView_uikit_wl_shapeCorner, DEFAULT_SHAPE_CORNER)

			waveColor =
				it.getColor(R.styleable.UIKitWaveLoadingView_uikit_wl_waveColor, Color.parseColor(DEFAULT_WAVE_COLOR))
			waveBackgroundColor = it.getColor(
				R.styleable.UIKitWaveLoadingView_uikit_wl_waveBackgroundColor,
				Color.parseColor(DEFAULT_WAVE_BACKGROUND_COLOR)
			)
			waveAmplitude = it.getFloat(R.styleable.UIKitWaveLoadingView_uikit_wl_waveAmplitude, DEFAULT_WAVE_AMPLITUDE)
			waveVelocity = it.getFloat(R.styleable.UIKitWaveLoadingView_uikit_wl_waveVelocity, DEFAULT_WAVE_VELOCITY)

			borderColor = it.getColor(
				R.styleable.UIKitWaveLoadingView_uikit_wl_borderColor,
				Color.parseColor(DEFAULT_BORDER_COLOR)
			)
			borderWidth =
				it.getDimension(R.styleable.UIKitWaveLoadingView_uikit_wl_borderWidth, dpTopx(DEFAULT_BORDER_WIDTH))

			process = it.getInteger(R.styleable.UIKitWaveLoadingView_uikit_wl_process, DEFAULT_PROCESS)

			textLocation =
				when (it.getInteger(R.styleable.UIKitWaveLoadingView_uikit_wl_textLocation, DEFAULT_TEXT_LOCATION)) {
					0 -> Location.FLOW
					1 -> Location.CENTER
					2 -> Location.TOP
					else -> Location.BOTTOM
				}
			text = it.getString(R.styleable.UIKitWaveLoadingView_uikit_wl_text) ?: ""
			if (TextUtils.isEmpty(text)) text = ""
			textSize = it.getDimension(R.styleable.UIKitWaveLoadingView_uikit_wl_textSize, spTopx(DEFAULT_TEXT_SIZE))
			textStrokeWidth = it.getDimension(
				R.styleable.UIKitWaveLoadingView_uikit_wl_textStrokeWidth,
				spTopx(DEFAULT_TEXT_STROKE_WIDTH)
			)
			textStrokeColor = it.getColor(
				R.styleable.UIKitWaveLoadingView_uikit_wl_textStrokeColor,
				Color.parseColor(DEFAULT_TEXT_STROKE_COLOR)
			)
			textColor =
				it.getColor(R.styleable.UIKitWaveLoadingView_uikit_wl_textColor, Color.parseColor(DEFAULT_TEXT_COLOR))
			isTextWave = it.getBoolean(R.styleable.UIKitWaveLoadingView_uikit_wl_textWave, false)
			isTextBold = it.getBoolean(R.styleable.UIKitWaveLoadingView_uikit_wl_textBold, false)
		}.recycle()
		...
		waveValueAnim.apply {
			...
			addUpdateListener(waveValueAnimUpdateListener)
		}
	}
	...
	fun startLoading() {
		if (!waveValueAnim.isStarted) {
			isWaveLoadingFlag = true
			waveValueAnim.addUpdateListener(waveValueAnimUpdateListener)
			waveValueAnim.start()
		}
	}

	fun cancelLoading() {
		isWaveLoadingFlag = false
		waveValueAnim.removeAllUpdateListeners()
		waveValueAnim.cancel()
	}

	fun pauseLoading() {
		if (waveValueAnim.isRunning) {
			waveValueAnim.pause()
		}
	}

	fun resumeLoading() {
		if (waveValueAnim.isPaused) {
